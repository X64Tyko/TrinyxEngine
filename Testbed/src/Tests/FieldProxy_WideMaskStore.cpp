...
TEST(FieldProxy_WideMaskStore)
{
	...
	{
		FieldProxy<float, FieldWidth::WideMask> proxy;
		proxy.Bind(data, flags, 0, 5); // 5 active lanes

		proxy = 9.9f;

		for (int i = 0; i < 5; ++i)
			ASSERT_EQ(data[i], 9.9f);
	...
	{
		alignas(32) float full[8] = {1, 2, 3, 4, 5, 6, 7, 8};
		FieldProxy<float, FieldWidth::WideMask> proxy;
	...
	{
		alignas(32) float single[8] = {0, 1, 2, 3, 4, 5, 6, 7};
		FieldProxy<float, FieldWidth::WideMask> proxy;
		...
		for (int i = 1; i < 8; ++i)
			ASSERT_EQ(single[i], static_cast<float>(i)); // unchanged
	}

	// --- Masked += ---
	{
		alignas(32) float addData[8] = {10, 10, 10, 10, 10, 10, 10, 10};
		FieldProxy<float, FieldWidth::WideMask> proxy;
